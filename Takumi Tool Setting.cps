...
properties = {
    //added post options for probing
    _optionallyCycleToolsAtStart: {
    ...
    _measureMethod: {
      ...
      description: "Macro selction for tool touch off. P9921 = GoProbe",
      type       : "enum",
      values     : [
        {title:"P9857", id:"P9857"},
...
function prepareForToolCheck() {
  ...
  //var probingType = getProbingType(tool.type);
...
//
function getProbingType(toolType){
  switch (toolType){
    //off center
...
function getSpinDirection(tool){
  //var spinRotaion
  if(tool.isClockwise() == true){
    //writeComment("CW");
    //spinRotaion = "";
    return "";
  }else if(tool.isClockwise() == false){
    //writeComment("CCW");
    //spinRotaion = "-"
    return "-";
  } else {
    //writeComment("Neither");
    //spinRotaion = "INVALID SPIN DIRECTION";
    return "Error";
  }
}

function writeToolMeasureBlock(tool, preMeasure) {
  //var spinRotaion = getSpinDirection(tool)
  ...
  if (true) { // Measure tool lengths using P9857 or P9921
    writeOptionalBlock(writeToolCycleBlock(tool))
    var probingType = getProbingType(tool.type);
    writeComment(localize("Touching off T" + toolFormat.format(tool.number) + " - " + getToolTypeName(tool.type) + " - " + probingType))


    //use Macro P9857 to measure tools - Contact Tool Setting package
    if (getProperty("_measureMethod") == "P9857") {
      //call tool height offfset
      writeOptionalBlock("G43 H"+ toolFormat.format(tool.number))
      ...
      writeFunction(
        ...
        "T" + toolFormat.format(tool.number), //T = offset number, defaults to current tool
        comment
      );
    }
    //GoProbe based cycle P9921
    if (getProperty("_measureMethod") == "P9921") {
      writeFunction(
        gFormat.format(65),
        "P9921",
        "M21",
...
function onOpen() {
  ...
  if (getProperty("_optionallyCycleToolsAtStart") || getProperty("_measureTools")) {
    var tools = getToolTable();
    if (tools.getNumberOfTools() > 0) {
      //writeln("");
      ...
      for (var i = 0; i < tools.getNumberOfTools(); ++i) {
        var tool = tools.getTool(i);
        if (getProperty("_measureTools") && (tool.type == TOOL_PROBE)) {
          continue;
        }
        writeln("(-----------------)");
        var comment = "T" + toolFormat.format(tool.number) + " " +
...
var smoothingSettings = {
  ...
  semifinishing         : 5, // semi-finishing level for smoothing in automatic mode - was 7
...
function subprogramStart(_initialPosition, _abc, _incremental) {
  if (getProperty("useFilesForSubprograms")) {
    var path = FileSystem.getCombinedPath(FileSystem.getFolderPath(getOutputPath()), "O"+currentSubprogram ); //+ "." + extension
    redirectToFile(path);
    writeln("%");
  } else {
    redirectToBuffer();
...
function onCommand(command) {
  ...
  case COMMAND_STOP_CHIP_TRANSPORT:
    return;
  case COMMAND_BREAK_CONTROL:
    if(tool.type == TOOL_PROBE) { //skip checking probes
      toolChecked = true;
    };
    if (!toolChecked) { // avoid duplicate COMMAND_BREAK_CONTROL
      prepareForToolCheck();
      probingType = getProbingType(tool.type);
      writeComment("TOOL BREAK CONTROL")
      writeComment(localize("Checking T" + toolFormat.format(tool.number) + " - " + getToolTypeName(tool.type) + " - " + probingType));
      ...
      writeBlock(
        ...
        "C0",
        "T" + toolFormat.format(tool.number),
        conditional((probingType == "off-center"), "D" + getSpinDirection(tool) + xyzFormat.format(tool.diameter)),
        //"B" + xyzFormat.format(0),
        "H" + xyzFormat.format(getProperty("_toolBreakageTolerance"))
      );
      toolChecked = true;
